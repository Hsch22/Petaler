...
class Tomato(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        try:
            ...
            self.setWindowFlags(
                ...
                Qt.WindowStaysOnTopHint |   # 窗口保持在顶部
                Qt.SubWindow                # 作为子窗口处理（影响任务栏、父子关系等）
            )
            # 设置窗口在关闭时自动删除，避免内存泄漏
        ...
        if os.path.exists(self._font_path):
            ...
            if font_id != -1:
                ...
                if font_families:
                    ...
                    print(f"字体 '{effective_font_family}' 从 '{self._font_path}' 加载成功。")
                else:
                    print(f"[警告] 字体文件 '{self._font_path}' 加载成功但无法获取字体家族名称，将使用 '{effective_font_family}'。")
            else:
                print(f"[警告] 无法从 '{self._font_path}' 加载字体 (文件可能无效)。将使用 '{effective_font_family}'。")
        else:
            print(f"[警告] 字体文件未找到: '{self._font_path}'。将使用 '{effective_font_family}'。")

        # 创建 QFont 对象，后续 UI 元素将使用此字体
        self._widget_font = QFont(effective_font_family, all_font_size)
    ...
    def _init_ui(self):
        ...
        n_tomato_label.setFont(self._widget_font)
    ...
    def confirm(self):
        ...
        try:
            ...
            print("[错误] Tomato.confirm: 无法访问 'self.n_tomato'。窗口可能未正确初始化。")
        except Exception as e:
            # 捕获信号发射过程中可能出现的其他错误
...
class Focus(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # 如果 '宋体' 不可用，系统会尝试使用默认字体
        self._font = QFont('宋体', all_font_size)
...
class Remindme(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # --- 配置常量 ---
        self._fixed_width = 450
        self._fixed_height = 300
        self._font = QFont('宋体', all_font_size) # 统一字体对象
    ...
    def _init_ui(self):
        ...
        #vbox_r.addStretch(1) # 移除最后的 stretch 使按钮靠近底部
    ...
    def _load_initial_text(self):
        ...
        try:
            ...
            print(f"[错误] Remindme._load_initial_text: 无法加载提醒文件 '{self._remind_file_path}': {e}")
            # 即使加载失败，也设置为空文本
            self.e2.setPlainText('')
        except Exception as e:
            print(f"[严重错误] Remindme._load_initial_text: 加载提醒时发生意外错误: {e}")
            self.e2.setPlainText('')
                ...
                for line in texts:
                    ...
                    if line.startswith('#重复'):
                        ...
                        if len(parts) >= 4:
                            ...
                            remind_text = reminder_parts[1] if len(reminder_parts) > 1 else ''

                            try:
                                minute_value = int(value_str)
                                if mode_str == '每到' and parts[3] == '分时':
                                    texts_to_emit.append(('repeat_point', 0, minute_value, remind_text))
                                elif mode_str == '每隔' and parts[3] == '分钟':
                                    texts_to_emit.append(('repeat_interval', 0, minute_value, remind_text))
                                else:
                                     print(f"[警告] initial_task: 无法解析重复行格式: {line}")
                            except ValueError:
                                print(f"[警告] initial_task: 重复行分钟值无效: {line}")
                            except Exception as e_parse:
                                print(f"[错误] initial_task: 解析重复行时出错 '{line}': {e_parse}")

            else:
                print(f"[信息] initial_task: 提醒文件 '{self._remind_file_path}' 不存在，无重复任务加载。")

        except IOError as e:
            print(f"[错误] Remindme.initial_task: 读取提醒文件 '{self._remind_file_path}' 时出错: {e}")
        except Exception as e:
        ...
        for args in texts_to_emit:
            try:
                self.confirm_remind.emit(*args)
                 ...
                 print(f"[错误] initial_task: 发射恢复的重复任务信号时出错 ({args}): {e_emit}")
        ...
        try:
            ...
            if self.checkA.isChecked(): # 模式A: 一段时间后
                ...
                time_torun = datetime(year=now.year, month=now.month, day=now.day,
                                      hour=hs, minute=ms, second=0) # 秒设为0更稳定
    ...
    def save_remindme(self):
        ...
        try:
            ...
            print(f"[错误] Remindme.save_remindme: 无法保存提醒文件 '{self._remind_file_path}': {e}")
        except Exception as e:
