...
from PyQt5.QtCore import (QEvent, QObject, QPoint, Qt, QThread, QTimer,pyqtSignal) 
...
try:
    ...
    screen_scale = 1.0
    print(f"警告：无法获取屏幕缩放比例，将使用默认值 1.0")


class PetWidget(QWidget):

    def __init__(self, parent: QWidget | None = None, curr_pet_name: str = '', pets: tuple = ()):
        """
        初始化宠物窗口部件。
        ...
        if not curr_pet_name and not pets:
            raise ValueError("必须提供 'curr_pet_name' 或非空的 'pets' 元组来初始化 PetWidget。")

        # --- 核心数据属性初始化 ---
        self.pets: tuple = pets  # 存储所有可用宠物的名称元组
        self.curr_pet_name: str = ''  # 当前激活的宠物名称 (将在 init_conf 中设置)
        self.pet_conf: PetConfig = PetConfig()  # 宠物配置对象 (将在 init_conf 中加载实际配置)
    ...
    def _handle_no_fall_on_release(self) -> None:
        ...
        try:
            if 'Animation' in self.workers:
                ...
                print(f"警告: 无法恢复动画，'Animation' worker 不存在于 self.workers 中。")
    ...
    def _init_ui(self):
        ...
        # 健康值进度条
        self.pet_hp = QProgressBar(self, minimum=0, maximum=100, objectName='PetHP') #设置objectName，便于样式表选择
    ...
    def _set_menu(self, pets=()):
        ...
        if self.pet_conf.random_act_name is not None:
            ...
            select_acts = [_build_act(name, act_menu, self._show_act) 
    ...
    def _change_pet(self, pet_name: str) -> None:
        ...
        if hasattr(self, '_setup_ui') and callable(getattr(self, '_setup_ui')):
             ...
             print(f"警告：方法 _setup_ui 未找到或不可调用。跳过 _change_pet 中的 UI 初始化步骤。 ")
    ...
    def _setup_ui(self, pic_dict):
        ...
        self.setFixedSize(int(self.pet_conf.width+self.margin_value),
    ...
    def set_img(self): #, img: QImage) -> None:
        ...
        try:
            ...
            print(f"错误：无法从 settings.current_img 获取尺寸。请确保它是一个有效的 QImage 对象。")
    ...
    def _change_status(self, status: str, change_value: float):
        ...
        # --------------------------
            ...
            if change_value > 0:
                ...
                print(f"(条件不满足：减少心情值，但 HP ({self.pet_data.current_hp}) >= 60)，操作被忽略。") 
    ...
    def _change_time(self, status, timeleft):
        ...
        # 定义有效的状态列表
        valid_statuses = [
            'tomato_start', 'tomato_rest', 'tomato', 'tomato_end',
        ...
        try:
            ...
            print(f"[错误] _change_time: 提供的 timeleft 值 ('{timeleft}') 无法转换为整数: {e}")
        ...
        try:
            ...
            if hasattr(thread, 'terminate') and callable(thread.terminate):
                thread.terminate()
                 ...
                 print(f"[错误] stop_thread: 线程对象 '{module_name}' 没有 terminate 方法。")
            ...
            if hasattr(thread, 'wait') and callable(thread.wait):
             ...
             print(f"[错误] stop_thread: 提供的模块名 '{module_name}' 在 workers 或 threads 字典中不存在。")
        except AttributeError as e:
             print(f"[错误] stop_thread: 尝试调用 kill/terminate/wait 时出错，对象可能不符合预期结构: {e}")
        except Exception as e:
    ...
    def fall_onoff(self):
        ...
        try:
            ...
            if sender is None:
                print("[错误] fall_onoff: 无法获取发送者对象 (sender is None)。操作中止。")
                return
                
            if not (hasattr(sender, 'text') and callable(sender.text) and \
                    hasattr(sender, 'setText') and callable(sender.setText)):
                print(f"[错误] fall_onoff: 发送者对象 '{sender}' 缺少 text 或 setText 方法。操作中止。")
                return
            ...
            if current_text == "禁用掉落":
                ...
                print(f"[错误] fall_onoff: 发送者对象的文本为非预期的 '{current_text}'。状态未改变。")

        except AttributeError as e:
             # 捕获访问 settings.set_fall 可能出现的属性错误
             print(f"[错误] fall_onoff: 访问 settings.set_fall 时出错: {e}。请确保 settings 对象及其属性已正确初始化。")
    ...
    def show_tomato(self):
        ...
        try:
            ...
            # 场景 2 & 3: 设置窗口不可见
                ...
                # 尝试调用取消方法
                if 'Scheduler' in self.workers and hasattr(self.workers['Scheduler'], 'cancel_tomato'):
                    self.workers['Scheduler'].cancel_tomato()
                else:
                    print("[警告] show_tomato: 无法取消番茄钟，因为 'Scheduler' worker 或其 'cancel_tomato' 方法未找到。")
                # 隐藏相关UI元素
                self.tomatoicon.hide()
                self.tomato_time.hide()
            ...
            print(f"[错误] show_tomato: 访问必要的UI元素或属性时出错: {e}。请检查是否所有相关对象 (tomato_window, tomato_clock, tomatoicon, tomato_time, workers) 都已正确初始化。")
        except KeyError as e:
            print(f"[错误] show_tomato: 无法访问 'Scheduler' worker: {e}。请确保 workers 字典中包含此键。")
        except Exception as e:
    ...
    def run_tomato(self, nt):
        ...
        try:
            ...
            # 3. 尝试启动番茄钟任务
            if 'Scheduler' in self.workers and hasattr(self.workers['Scheduler'], 'add_tomato'):
                try:
                    num_tomatoes = int(nt) # 尝试转换输入为整数
                    self.workers['Scheduler'].add_tomato(n_tomato=num_tomatoes)
                except (ValueError, TypeError):
                    print(f"[错误] run_tomato: 无法启动番茄钟，因为提供的数量 '{nt}' 不是有效的整数。")
                    return # 停止执行后续步骤
            else:
                    print("[警告] run_tomato: 无法启动番茄钟，因为 'Scheduler' worker 或其 'add_tomato' 方法未找到。")
                    return
            ...
            self.tomato_time.show()
             ...
             print(f"[错误] run_tomato: 访问必要的UI元素或属性时出错: {e}。请检查是否所有相关对象 (tomato_clock, tomato_window, tomatoicon, tomato_time, workers) 都已正确初始化。")
        except KeyError as e:
             print(f"[错误] run_tomato: 无法访问 'Scheduler' worker: {e}。请确保 workers 字典中包含此键。")
        except Exception as e:
    ...
    def change_tomato_menu(self):
        ...
        try:
            ...
            self.tomato_time.hide()
             ...
             print(f"[错误] change_tomato_menu: 访问必要的UI元素 (tomato_clock, tomatoicon, tomato_time) 时出错: {e}。请检查它们是否已正确初始化。")
        except Exception as e:
    ...
    def show_focus(self):
        ...
        try:
            ...
            required_attrs = ['focus_window', 'focus_clock', 'focusicon', 'focus_time', 'workers']
            for attr in required_attrs:
                if not hasattr(self, attr):
                    print(f"[错误] show_focus: 缺少必要的属性 'self.{attr}'。操作中止。")
                    return
            if 'Scheduler' not in self.workers:
                 print(f"[错误] show_focus: 'self.workers' 字典中缺少 'Scheduler' 键。操作中止。")
                 return
            ...
            # 场景 2 & 3: 设置窗口不可见
                ...
                if hasattr(scheduler, 'cancel_focus') and callable(scheduler.cancel_focus):
                    scheduler.cancel_focus()
                else:
                    print("[警告] show_focus: 无法取消专注任务，因为 'Scheduler' worker 没有 'cancel_focus' 方法。")
             ...
             print(f"[错误] show_focus: 访问UI元素的方法或属性时出错: {e}。请确保对象已正确初始化且类型正确。")
        except KeyError as e:
    ...
    def run_focus(self, task: str, hs, ms):
        ...
        try:
            ...
            required_attrs = ['focus_clock', 'focus_window', 'focusicon', 'focus_time', 'workers']
            for attr in required_attrs:
                if not hasattr(self, attr):
                    print(f"[错误] run_focus: 缺少必要的属性 'self.{attr}'。操作中止。")
                    return
            if 'Scheduler' not in self.workers:
                 print(f"[错误] run_focus: 'self.workers' 字典中缺少 'Scheduler' 键。操作中止。")
                 return
            scheduler = self.workers['Scheduler']
            if not (hasattr(scheduler, 'add_focus') and callable(scheduler.add_focus)):
                 print(f"[错误] run_focus: 'Scheduler' worker 没有 'add_focus' 方法。操作中止。")
                 return
            ...
            if self.focus_clock.text() == "专注时间":
                ...
                if task not in ['range', 'point']:
                    print(f"[错误] run_focus: 无效的任务类型 '{task}'。任务类型必须是 'range' 或 'point'。")
                    return # 中止执行
             ...
             print(f"[错误] run_focus: 访问UI元素的方法或属性时出错: {e}。请确保对象已正确初始化且类型正确。")
        except KeyError as e:
             # 这个理论上被入口检查覆盖了，但保留
             print(f"[错误] run_focus: 访问 'Scheduler' worker 时出错: {e}。")
        except TypeError as e:
             # 捕获调用 add_focus 时可能因 hs, ms 类型不匹配引发的错误
             print(f"[错误] run_focus: 调用 add_focus 时参数类型错误: {e}。请检查 hs ('{hs}') 和 ms ('{ms}') 是否为预期类型。")
        except Exception as e:
    ...
    def change_focus_menu(self):
        ...
        try:
            ...
            for attr in required_attrs:
                if not hasattr(self, attr):
                    print(f"[错误] change_focus_menu: 缺少必要的属性 'self.{attr}'。操作中止。")
                    return
            if not (hasattr(self.focus_clock, 'text') and callable(self.focus_clock.text) and \
                    hasattr(self.focus_clock, 'setText') and callable(self.focus_clock.setText)):
                 print(f"[错误] change_focus_menu: 'self.focus_clock' 对象缺少 text 或 setText 方法。操作中止。")
                 return
            if not (hasattr(self.focusicon, 'hide') and callable(self.focusicon.hide) and \
                    hasattr(self.focus_time, 'hide') and callable(self.focus_time.hide)):
                 print(f"[错误] change_focus_menu: 'self.focusicon' 或 'self.focus_time' 对象缺少 hide 方法。操作中止。")
                 return
            ...
            if self.remind_window.isVisible():
                ...
                try:
                    ...
                    if current_pos is None:
                         print(f"[错误] show_remind: 'self.pos()' 返回了 None。无法计算窗口位置。")
                         return # 中止

                    # 检查 current_pos 是否有 x, y 方法
                    if not (hasattr(current_pos, 'x') and callable(current_pos.x) and \
                            hasattr(current_pos, 'y') and callable(current_pos.y)):
                        print(f"[错误] show_remind: 'self.pos()' 返回的对象缺少 x 或 y 方法。无法计算窗口位置。")
                        return
                    ...
                    self.remind_window.show()
                     ...
                     print(f"[错误] show_remind: 在计算窗口位置时发生类型或值错误: {e}。检查 width/height 和 pos 的返回值。")
                except Exception as e:
                     print(f"[错误] show_remind: 在尝试定位并显示提醒窗口时发生错误: {e}")
    ...
    def run_remind(self, task_type, hs=0, ms=0, texts=''):
        ...
        try:
            ...
            if not hasattr(self, 'workers') or not isinstance(self.workers, dict):
                 print(f"[错误] run_remind: 'self.workers' 字典不存在或类型错误。操作中止。")
                 return

            if 'Scheduler' not in self.workers:
                 print(f"[错误] run_remind: 'self.workers' 字典中缺少 'Scheduler' 键。操作中止。")
                 return
            scheduler = self.workers['Scheduler']

            if not (hasattr(scheduler, 'add_remind') and callable(scheduler.add_remind)):
                 print(f"[错误] run_remind: 'Scheduler' worker 没有 'add_remind' 方法。操作中止。")
                 return
            ...
            if task_type == 'range':
                ...
                print(f"[错误] run_remind: 无效的任务类型 '{task_type}'。无法添加提醒。")

        except TypeError as e:
             # 捕获调用 add_remind 时因参数类型错误 (hs, ms, texts, repeat) 引发的异常
             print(f"[错误] run_remind: 调用 add_remind 时参数类型错误: {e}。检查 hs='{hs}', ms='{ms}', texts='{texts}' 类型。")
        except Exception as e:
    ...
    def runAnimation(self):
        ...
        try:
            ...
            if 'Animation_worker' not in globals() and 'Animation_worker' not in locals():
                 print(f"[错误] runAnimation: 'Animation_worker' 类未定义或导入。无法创建工作者。")
                 # 清理已创建的线程对象
            ...
            else: print(f"[警告] runAnimation: Animation_worker 缺少 sig_setimg_anim 信号。")
            
            if hasattr(worker, 'sig_move_anim'):
                worker.sig_move_anim.connect(self._move_customized)
            else: print(f"[警告] runAnimation: Animation_worker 缺少 sig_move_anim 信号。")
                
            if hasattr(worker, 'sig_repaint_anim'):
                worker.sig_repaint_anim.connect(self.repaint)
            else: print(f"[警告] runAnimation: Animation_worker 缺少 sig_repaint_anim 信号。")
            ...
            print(f"[错误] runAnimation: 访问字典 key 时出错: {e}。可能发生在线程/worker未成功创建。")
        except TypeError as e:
            # 例如，connect 时信号或槽不匹配，或 moveToThread 参数错误
            print(f"[错误] runAnimation: 类型错误: {e}。可能发生在信号槽连接或对象移动时。")
        except Exception as e:
            # 捕获其他所有意外错误
            print(f"[错误] runAnimation: 启动动画线程时发生未知错误: {e}")
            # 尝试清理，防止留下部分初始化的状态
            if module_name in self.threads: del self.threads[module_name]
            if module_name in self.workers: del self.workers[module_name]
    ...
    def runInteraction(self):
        ...
        try:
            ...
            else: print(f"[警告] runInteraction: Interaction_worker 缺少 sig_setimg_inter 信号。")

            if hasattr(worker, 'sig_move_inter'):
                 worker.sig_move_inter.connect(self._move_customized)
            else: print(f"[警告] runInteraction: Interaction_worker 缺少 sig_move_inter 信号。")

            if hasattr(worker, 'sig_act_finished'):
                 worker.sig_act_finished.connect(self.resume_animation)
            else: print(f"[警告] runInteraction: Interaction_worker 缺少 sig_act_finished 信号。")
    ...
    def runScheduler(self):
        ...
        try:
            ...
            self.workers[scheduler_module_name].moveToThread(self.threads[interaction_thread_name]) 

            # 3. 连接调度器线程的 started 到 run (可能行为异常，见步骤3注释)
            self.threads[scheduler_module_name].started.connect(self.workers[scheduler_module_name].run)
            ...
            for signal_name, slot_func in signals_to_connect.items():
                if hasattr(worker, signal_name):
                    getattr(worker, signal_name).connect(slot_func)
                     ...
                     print(f"[警告] runScheduler: Scheduler_worker 缺少 {signal_name} 信号。")
            ...
            print(f"[错误] runScheduler: 访问字典 key 时出错: {e} (可能是 '{interaction_thread_name}' 线程不存在，或线程/worker未成功创建)。")
        except TypeError as e:
            print(f"[错误] runScheduler: 类型错误: {e}。")
        except Exception as e:
            print(f"[错误] runScheduler: 启动计划任务线程时发生未知错误: {e}")
            if scheduler_module_name in self.threads: del self.threads[scheduler_module_name]
            if scheduler_module_name in self.workers: del self.workers[scheduler_module_name]
        ...
        try:
            ...
            # 判断是否接触或低于地面
                ...
                try:
                    ...
                    print(f"[错误] _move_customized: 访问 settings 或 pet_conf 属性时出错: {e}。落地状态更新失败。")
                except IndexError as e:
                     print(f"[错误] _move_customized: 访问默认图片列表时索引错误 (可能列表为空): {e}。落地状态更新失败。")
                except Exception as e:
            ...
            print(f"[错误] _move_customized: 计算新位置时发生类型或值错误: {e}。输入: plus_x={plus_x}, plus_y={plus_y}")
        except AttributeError as e:
...
def _get_q_img(img_path: str) -> QImage | None:
    ...
    try:
        ...
        if image.load(img_path):
            ...
            print(f"[警告] _get_q_img: 无法加载图片 (可能格式不支持或文件损坏): '{img_path}'")
